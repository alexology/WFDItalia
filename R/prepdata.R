...
data.prep <- function(x){
    ...
    a <- select.list(c("1. Rete Surber", "2. Substrati artificiali"), title="TIPO DI CAMPIONAMENTO")
    ...
    t_1 <- colnames(t)
    if(a=="2. Substrati artificiali"){ #1
    		data.mts <- as.character(mts_t[,2])
    		mts_d <- match(t_1, data.mts)
    		if(length(which(is.na(mts_d)))==ncol(x)){#1.1
    			b <- select.list(c("1. Procedere", "2. Non Procedere"), title="Nessuna Unità Operazionale!")
        		if(b=="2. Non Procedere"){#1.1.1.c
    				stop("annullato dall'Utente")
			}
			if(b=="1. Procedere"){#1.1.2
    		...
    		if(length(which(is.na(mts_d)))!=ncol(x)){#1.
   			...
   			com_ou <- intersect(mts_t[,2],colnames(mts_taxa))
    			...
    			if(nrow(x)==1){
    				mts_ept_1 <- aggregate(. ~ mts_ept[,1], data=mts_ept,sum)
    			...
    			if(nrow(x)>1) {
    				mts_ept_1 <- aggregate(. ~ mts_ept[,1], data=mts_ept[,-1],sum)
    				rownames(mts_ept_1) <- mts_ept_1[,1]
    				mts_ept_2 <- mts_ept_1[,-1]
    			}
    			...
    			if (length(r1) == 0) {
      			...
      			r_in <- which(rownames(aspt_v) %in% colnames(t))
				aspt_v1 <- aspt_v[r_in,]
     				mts_ept <- data.frame(com_ou2[,3], mts_ept[,-1])
