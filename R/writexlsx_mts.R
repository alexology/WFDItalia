writexlsx_mts <- function(X){
z <- "1. Sì"
nam <- list.files()
if(sum(match(nam,"STAR_ICMi.xlsx"),na.rm=T)==1){
z <- select.list(c("1. S?", "2. No"), title="Sovrascivere STAR_ICMi.xls?")
}
...
ref.met <-  X[[1]][1:8]
names(ref.met) <- c("ASPT", "N_Fam", "N_EPT_Fam", "1-GOLD", "Diversità di Shannon","log SelEPTD", "STAR_ICMi", "MTS")
ref.cla <- X[[1]][9:12]
...
addDataFrame(ref.met, sheet, startRow=1, startColumn=1, row.names=F, colnamesStyle=cs2, colStyle=list("1"=cs,"2"=cs,"3"=cs,"4"=cs,"5"=cs,"6"=cs,"7"=cs,"8"=cs))
addDataFrame(ref.cla, sheet, startRow=4, startColumn=1, row.names=F, colnamesStyle=cs2, colStyle=list("1"=cs,"2"=cs,"3"=cs,"4"=cs))
addDataFrame(X[[2]], sheet, startRow=9, startColumn=1, rownamesStyle=cs1, colnamesStyle=cs2, colStyle=list("1"=cs,"2"=cs,"3"=cs,"4"=cs,"5"=cs,"6"=cs,"7"=cs,"8"=cs,"9"=cs,"10"=cs,"11"=cs))
rowm1 <-  nrow(X[[2]])
cb <- CellBlock(sheet, 10, 11, rowm1, 1, create=T)
CB.setColData(cb, 11, 1)
